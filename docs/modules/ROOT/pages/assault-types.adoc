...
The delay is applied via `Thread.sleep()` in the JAX-RS filter, before the request reaches your endpoint logic. On a Vert.x event-loop thread (a non-blocking endpoint) the delay is skipped, with a `WARN` logged once, rather than blocking the loop.

If `min-milliseconds` is greater than `max-milliseconds`, the values are swapped (with a `WARN` log) at startup and when the range comes from JSON-RPC, an import or the state file; the Dev UI form refuses an inverted range before saving.
...
[source,properties]
----
quarkus.goblin.assault.exception.type=java.lang.IllegalStateException
...
If the class cannot be found, does not extend `RuntimeException`, or lacks a `String` constructor, an `ERROR` is logged at startup (and when the class is saved from the Dev UI or JSON-RPC, where a warning reports it), and the engine falls back to `RuntimeException`.
...
The status code must be in the `100`-`599` range. Out-of-range values are detected at startup (and when they come from JSON-RPC, an import or the state file) and default to `503` with an `ERROR` log; the Dev UI form refuses them before saving.
...
* **`INFLATE`** -- pads the body with a fixed `[goblin-response-inflated]` marker so the effective size reaches `percentage`% of the original (typically over `100`%, e.g. `200`). Padding is capped at 1 MiB added to the original body, whatever the percentage. The `Content-Length` header keeps advertising the original, smaller length while more bytes are emitted, exercising length-validating proxies and parsers. The transformed bytes are written verbatim: truncation never re-encodes a split multibyte character, so the delivered size always matches the configured percentage.

The `percentage` is validated per mode: `0`-`100` for `TRUNCATE`, `101`-`1000` for `INFLATE`. Out-of-range values are clamped with a `WARN` log at startup and when they come from JSON-RPC, an import or the state file (e.g. `INFLATE` with `50` becomes `101`); the Dev UI form refuses them before saving. In both modes the target size is `floor(originalBytes * percentage / 100)`: the emitted size is rounded down so it never exceeds the requested percentage, and any non-zero percentage keeps at least one byte (`0` yields an empty body).

NOTE: The response body assault rewrites the entity in the JAX-RS response filter, before it is serialized, so it only applies to `String`, `CharSequence` and `byte[]` entities. An object entity serialized by a JSON provider (a POJO, a record, a collection...) is left untouched, without any log nor history entry: to corrupt a JSON payload, return it as a `String` or a `byte[]`. It follows the same targeting rules as the other server-side assaults (target level and package/annotation filters), and it is recorded in the assault history like any other assault.
...
The header rules are applied in the response phase, **after** any response body transformation, so a `SET` on `Content-Length` or `Content-Type` wins over the values computed by the body assault. Header names are matched case-insensitively (as per HTTP), so a rule replaces or removes a header emitted with any casing. Header names must be HTTP tokens (letters, digits and ``!#$%&'*+-.^_`|~``) and values must not contain CR, LF or control characters. An invalid rule never implicitly becomes `SET`, and how it is reported depends on where it comes from:

* in `application.properties`, an unknown `action` is a configuration error that fails the startup, and an invalid name or value is ignored with a `WARN` log;
* in the Dev UI or JSON-RPC, the rule is rejected (`ok=false`, or skipped with a `warning` in an import);
* in `.goblin-state.json`, the rule is skipped with a `WARN` log.

The legacy `ADD` and `OVERRIDE` actions of earlier versions are accepted as `SET` in the Dev UI, JSON-RPC and the state file, not in `application.properties`.
...
The assault types above are applied by the `HTTP_IN` layer. The other layers only inject **latency** and **exception**, re-using the same toggles, latency range and exception class/message; `target.level` applies to every layer. A layer only fires when its check-box is armed in the xref:dev-ui.adoc[Dev UI `Chaos layers`] group (see xref:how-it-works.adoc#chaos-layers[Chaos layers]).
...
Beyond the HTTP boundaries, Goblin can inject faults **inside** application beans via the SERVICE layer (see xref:how-it-works.adoc#service-layer-assaults[Service-layer assaults] for the mechanics). When the request resolves to the SERVICE layer, a CDI interceptor applies the same two primitives on the guarded bean method:
...
Service-layer assaults re-use the same latency range and exception class/message configured for the incoming side, and the same `latency`/`exception` toggles. They fire only when the **SERVICE** check-box is armed in the xref:dev-ui.adoc[Dev UI `Chaos layers`] group.

NOTE: Service assaults apply to *eligible bean methods* (see xref:configuration-reference.adoc[targeting rules]): JAX-RS resources, `@Provider`s, interfaces, static/private methods and `@Fallback` fallback targets are never decorated. Each assault is recorded in the history with a `fully.qualified.ClassName.method` identifier. The interceptor is woven at build time and the crafted binding is inactive outside dev/test builds.

[#database-layer-assaults]
== Database-layer assaults (JDBC connections)

When the request -- or the consumed message -- resolves to the **DATABASE** layer, the JDBC connection acquisition fails or is delayed, exactly where an unreachable or slow database surfaces (see xref:how-it-works.adoc#database-layer-assaults[Database-layer assaults] for the mechanics):
...
Each message consumed by an `@Incoming` method is its own pseudo-request: it resolves among the `DATABASE`, `MESSAGING` and `SERVICE` layers, so a database or service fault can also surface while a message is processed (see xref:how-it-works.adoc#messaging-layer-assaults[Messaging-layer assaults] for the mechanics). When **MESSAGING** wins:

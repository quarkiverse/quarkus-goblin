...
In addition, **client-side assaults** (latency and exception) can be enabled for outgoing MicroProfile REST Client calls -- see xref:assault-types.adoc#client-side-assaults[Client-side assaults]. The response body and response header assaults are applied when the response is emitted, so they compose with every other type: for example, HTTP status + response body turns a 503 into a 503 with a truncated body, and HTTP status + response header adds a custom header to the 503.

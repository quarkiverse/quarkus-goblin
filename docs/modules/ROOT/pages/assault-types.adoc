...
Goblin supports five types of server-side chaos assaults, plus client-side assaults for outgoing REST Client and Vert.x WebClient calls, and service-layer assaults on business bean method invocations. Each type has its own **boolean toggle** -- you can enable multiple types simultaneously on the same request.

...
* **Chaos layers** -- One check-box per chaos layer (`DATABASE`, `MESSAGING`, `SERVICE`, `HTTP_OUT`, `HTTP_IN`) shown in ascending order next to the level slider. Only the checked layers are candidates each request; the deepest layer whose probability roll passes wins for that request (see xref:how-it-works.adoc#chaos-layers[Chaos layers]). `DATABASE` and `MESSAGING` are displayed as *soon* until their hooks land. By default `HTTP_IN` and `HTTP_OUT` are armed, which keeps the pre-layers behaviour.

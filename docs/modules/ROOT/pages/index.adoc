...
| `getStatus`
| Returns `active`, the active `profile`, the four assault toggles, and the target `level`.

| `getConfig`
| Returns the full mutable config (profile, toggles, latency range, exception config, HTTP status config, level).

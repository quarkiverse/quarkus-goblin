...
* **Multi-layer.** Chaos resolves per request against the armed layers (`DATABASE` → `MESSAGING` → `SERVICE` → `HTTP_OUT` → `HTTP_IN`), so a fault injected deepest propagates up through the resilience mechanisms at every boundary.

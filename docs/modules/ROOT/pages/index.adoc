...
No. The chaos implementation lives in the `quarkus-goblin-dev` runtime module, which is only attached in dev mode. In a production build the artifacts are physically absent -- there is no configuration that can re-enable them.

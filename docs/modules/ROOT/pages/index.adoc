...
In addition, **client-side assaults** (latency and exception) can be enabled for outgoing MicroProfile REST Client calls -- see xref:index.adoc#client-side-assaults[Client-side assaults].
...
| `AssaultType`
| `LATENCY`
| Initial assault type enabled at startup. Values: `LATENCY`, `EXCEPTION`, `HTTP_STATUS`, `DEPENDENCY_DEGRADATION`. Can be changed at runtime via the Dev UI. Ignored when a non-`NONE` profile is selected.
...
* the assault **type** (latency, exception, http-status, dependency-degradation)
...
Newest assaults appear first. A **summary band** above the table shows the totals per assault type and the average injected latency, updated live.

A **filter bar** narrows the list: filter by assault type, by a method text search, or by time period (last 5 minutes, 30 minutes, 1 hour, 24 hours, or all time). The counter shows how many of the total assaults match the current filters.
...
* the current assault configuration (active profile, latency, exception, HTTP status, dependency degradation toggles and values)
...
The persisted state only covers the **assault configuration** (profile, toggles, latency range, exception settings, HTTP status settings, target level). The global **enabled/active** flag is deliberately excluded: it is always taken from `quarkus.goblin.enabled` at startup and, during a session, from the Dev UI toggle -- a state file can never reactivate an extension you disabled via `quarkus.goblin.enabled=false`.
...
| `getStatus`
| Returns `active`, the active `profile`, the four assault toggles, the two client-side assault toggles, and the target `level`.

| `getConfig`
| Returns the full mutable config (profile, toggles, client-side toggles, latency range, exception config, HTTP status config, level).

...
* **Multi-layer.** Chaos resolves per request (or per consumed message) against the armed layers (`DATABASE` → `MESSAGING` → `SERVICE` → `HTTP_IN`, with `HTTP_OUT` rolled per outgoing call), so a fault injected deepest propagates up through the resilience mechanisms above it.
...
* xref:assault-types.adoc[] -- the six server-side chaos assaults (latency, exception, HTTP status, dependency degradation, response body, response header), service-layer assaults on business beans, plus client-side assaults for outgoing REST Client and Vert.x WebClient calls.
...
* xref:tracing.adoc[] -- optional OpenTelemetry spans for each assault (`quarkus-goblin-opentelemetry`).
* xref:compatibility.adoc[] -- coexistence with MicroProfile Fault Tolerance (service-layer interceptor placement), Quarkus REST, the REST clients and the optional database and messaging hooks.
...
Yes. Runtime changes are automatically saved to a `.goblin-state.json` file in the project working directory and restored on the next startup. Add `.goblin-state.json` to your `.gitignore`: it holds your local chaos settings, and a committed copy would silently replace the `application.properties` configuration of your teammates in dev mode.

...
A real-time log of every assault triggered, including the endpoint method, assault type, timestamp, and the actual applied latency duration (when the assault was a latency injection). Clear the history between test sessions.
...
* the full assault history, including the actual latency duration applied for each latency assault
...
| `getHistory`
| Return the assault history buffer. Each entry exposes the method, type, timestamp, and the applied latency duration (`latencyMs`, 0 for non-latency assaults).

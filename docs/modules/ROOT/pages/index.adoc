...
* **Dev and test only.** Chaos only activates under `quarkus:dev` and `@QuarkusTest`. In a production build the engine stays inactive whatever the configuration, the Dev UI is absent, and no application bean is woven with the service-assault interceptor; only the (inert) runtime classes ship in the extension jar.
...
* **Multi-layer.** Chaos resolves per request against the armed layers (`SERVICE` → `HTTP_IN`, with `HTTP_OUT` rolled per outgoing call), so a fault injected deepest propagates up through the resilience mechanisms above it. `DATABASE` and `MESSAGING` are reserved and not implemented yet.
...
* xref:compatibility.adoc[] -- coexistence with MicroProfile Fault Tolerance (service-layer interceptor placement) and both JAX-RS runtimes.

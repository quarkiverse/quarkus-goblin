...
| `server` for server-side assaults, `rest-client` for outgoing REST Client calls, `webclient` for outgoing Vert.x WebClient calls, `database` for JDBC connection acquisitions, `messaging` for `@Incoming` consumers.

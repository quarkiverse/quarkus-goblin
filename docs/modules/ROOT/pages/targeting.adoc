...
The exclusion applies to every layer that uses the targeting rules: a bean method carrying one of these annotations is not woven for `SERVICE` either (nor an `@Incoming` method for `MESSAGING`). To exercise the Fault Tolerance annotations themselves while they are excluded, arm the `DATABASE` layer, whose hook ignores the targeting rules, or let `SERVICE` hit a bean the guarded method calls that does not carry the annotation: the fault then fires inside the guarded method. Without the exclusion, `SERVICE` injects directly inside the guarded method (see xref:end-to-end-example.adoc[End-to-end example]).

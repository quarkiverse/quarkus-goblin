...
* `SERVICE` armed -- the business bean interceptor fires (see below); the HTTP filters stand down.
...
* **Eligibility.** A bean method is decorated only when the class belongs to the application root archive and the usual
  ...
  HTTP_IN layer and sit *above* the guarded beans. Methods named as the `fallbackMethod` of a `@Fallback` in the same 
  class are skipped too, so a fallback can answer the original failure instead of being assaulted itself.
* **Synchronous scope (for now).** Assaults run inside the guarded invocation, so `@Asynchronous` / non-blocking methods

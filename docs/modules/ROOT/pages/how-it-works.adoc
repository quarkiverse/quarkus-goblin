...
* **Synchronous scope (for now).** The decision is bound to the request thread, so `@Asynchronous` / non-blocking methods
  ...
  blocking the loop.
* **History.** Each service assault is recorded like any other, with a fully qualified `com.example.ClassName.method` descriptor -- including the

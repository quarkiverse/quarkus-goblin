...
. In *Chaos layers*, check **Service** (the default *Inbound REST* and *Outbound HTTP* can stay checked: the deepest armed layer wins).
...
----
. The call fails after about 0.4 s instead of 0.6 s: `@Timeout` interrupted the injected delay. The *Assault History* shows a `latency` entry on `io.quarkiverse.goblin.it.SampleService.timed`.
